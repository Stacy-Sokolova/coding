...
public:
	...
	unordered_map<pair<string, string>, int, pair_hash> stopdist;
};
...
struct InputReq : ModifyRequest {
	...
	void ParseDist(string_view s) {
		int d = stoi(ReadToken(s, "m").data());
		string_view to = ReadToken(s, "to");
		while (s.front() == ' ')
			s.remove_prefix(1);
		while (s.back() == ' ')
			s.remove_suffix(1);
		dist.push_back({ string(s),d });
	}
	...
	void ParseFrom(string_view input) override {
		...
		if (type == "Bus") {
			...
			y = stof(ReadToken(input, ",").data());

			while (!input.empty()) {
				ParseDist(ReadToken(input, ","));
			}
		}
	}
	...
	void Process(RouteManager& manager) override {
		if (type == "Bus") {
			...
			for (auto& stop : dist) {
				if (manager.stopdist.find({ name, stop.first }) == manager.stopdist.end())
					manager.stopdist.insert({ {name, stop.first}, stop.second });
				else
					manager.stopdist.at({ name, stop.first }) = stop.second;

				if (manager.stopdist.find({ stop.first, name }) == manager.stopdist.end())
					manager.stopdist.insert({ {stop.first, name}, stop.second });
			}
		}
	}
	...
	string Process(const RouteManager& manager) const override {
		...
		if (pos == manager.buses.end()) {
			...
			while (start != prev(manager.buses.at(bus_num).second.end())) {
				const auto& rhs = manager.stops.at(*start);
				...
				L += manager.stopdist.at({ first, *start });
			}

			if (manager.buses.at(bus_num).first == '-') {
				length *= 2;

				auto tail = prev(manager.buses.at(bus_num).second.end());
				while (tail != manager.buses.at(bus_num).second.begin()) {
					const auto& x = *tail;
					tail--;
					L += manager.stopdist.at({ x, *tail });
				}
			}
